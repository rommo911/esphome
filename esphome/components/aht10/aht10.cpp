...
void AHT10Component::setup() {
  ...
  if (this->write(&data, 1) != i2c::ERROR_OK) {
    ESP_LOGD(TAG, "Communication with AHT10 write failed!");
    this->mark_failed();
    return;
  }
  delay(AHT10_DEFAULT_DELAY * 5 );
  if (this->read(&data, 1) != i2c::ERROR_OK) {
    ESP_LOGD(TAG, "Communication with  AHT10 data failed!");
    this->mark_failed();
    return;
  }
  if (this->read(&data, 1) != i2c::ERROR_OK) {
    ESP_LOGD(TAG, "Communication with AHT10 read 2 failed!");
    this->mark_failed();
    return;
  }
